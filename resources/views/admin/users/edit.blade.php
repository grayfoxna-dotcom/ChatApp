        ...
        <div class="card">
            <div class="card-header">
                <div class="row align-items-center">
                    ...
                    <div class="col-auto"> 
                        <a href="{{ route('admin.users.index') }}" class="btn btn-secondary btn-sm"><i class="fas fa-arrow-left me-1"></i> Quay lại</a>           
                    </div><!--end col-->
                </div>  <!--end row-->                                  
            </div><!--end card-header-->
                ...
                <form id="editUserForm" action="{{ route('admin.users.update', $user->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="col-lg-8 border-end">
                            <div class="p-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="email" class="form-control bg-light" id="email" name="email" value="{{ old('email', $user->email) }}" readonly>
                                </div>
                                <div class="mb-3">
                                    <label for="isActive" class="form-label">Trạng thái tài khoản</label>
                                    <select class="form-select @error('isActive') is-invalid @enderror" id="isActive" name="isActive" required>
                                        <option value="1" {{ old('isActive', $user->isActive) == '1' ? 'selected' : '' }}>Hoạt động (Active)</option>
                                        <option value="0" {{ old('isActive', $user->isActive) == '0' ? 'selected' : '' }}>Bị khóa (Blocked)</option>
                                    </select>
                                    @error('isActive')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                        ...
                        <div class="col-lg-4">
                            <div class="p-3 text-center">
                                ...
                                <div class="position-relative d-inline-block mb-3">
                                    <img id="avatar-preview" src="{{ $user->avatar }}" alt="avatar" class="img-thumbnail rounded-circle thumb-xl" style="width: 150px; height: 150px; object-fit: cover;">
                                </div>
                    ...
                    <div class="row mt-3">
                        <div class="col-12 text-end">
                            <button type="button" class="btn btn-primary px-4" onclick="showConfirmModal()">Cập nhật người dùng</button>
                        </div>
                    </div>
                </form>
            </div><!--end card-body-->
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                <p class="mt-3">Bạn có chắc chắn muốn cập nhật thông tin cho người dùng này không?</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Hủy bỏ</button>
                <button type="button" class="btn btn-primary btn-sm" onclick="document.getElementById('editUserForm').submit();">Xác nhận cập nhật</button>
            </div>
        </div>
    </div>
</div>
