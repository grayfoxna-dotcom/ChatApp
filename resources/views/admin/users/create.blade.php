...
<style>
    ...
    /* Đảm bảo modal hiển thị trên các thành phần khác */
    #cropModal {
        z-index: 1200;
    }
</style>
                ...
                <form id="createUserForm" action="{{ route('admin.users.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="p-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('isActive') is-invalid @enderror" id="isActive" name="isActive" required>
                                        <option value="1" {{ old('isActive', '1') == '1' ? 'selected' : '' }}>Hoạt động (Active)</option>
                                        <option value="0" {{ old('isActive') == '0' ? 'selected' : '' }}>Bị khóa (Blocked)</option>
                                    </select>
                                    @error('isActive')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                    ...
                    <div class="row mt-3">
                        <div class="col-12 text-end">
                            <button type="button" class="btn btn-primary px-4" onclick="showConfirmModal()">Lưu người dùng</button>
                        </div>
                    </div>
                </form>
            </div><!--end card-body-->
...
<div class="modal fade" id="confirmModal" tabindex="-1" aria-labelledby="confirmModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-primary">
                <h6 class="modal-title m-0 text-white" id="confirmModalLabel">Xác nhận thêm người dùng</h6>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body text-center">
                <p class="mt-3">Bạn có chắc chắn muốn thêm người dùng mới với các thông tin đã nhập không?</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Hủy bỏ</button>
                <button type="button" class="btn btn-primary btn-sm" onclick="document.getElementById('createUserForm').submit();">Xác nhận thêm</button>
            </div>
        </div>
    </div>
</div>
